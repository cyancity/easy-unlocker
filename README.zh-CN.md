...
每次 `easyGet` 现场生成一把一次性钥匙，公钥随请求送到手机，手机用它把值封成密文，只有你这次命令手里的私钥能开。**中转服务器从头到尾只见密文。**审批发生在第二台设备上，不是发起请求那台机器上的一个弹窗，所以 Agent 跑在服务器或 CI 上、根本没有屏幕可弹时，这条路依然通。
...
> 它不是密码管理器，不替代 Bitwarden 或 1Password，不做浏览器自动填充，也不用买订阅。库就在手机里，只存你愿意放给 Agent 的那几条。
...
- 审批发生在第二台设备上，不是发起请求的机器上的一个弹窗：被污染的进程伪造不了、也替你点不掉；服务器和 CI 上没有屏幕的 Agent 也能找到人
